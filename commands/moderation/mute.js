        ...
        if (message.member.roles.cache.some(role => role.id === `${roles.staff}`)) {
            ...
            if (user) {
                ...
                if (member) {
                    ...
                    const embed = new Discord.MessageEmbed().setColor('ff0000').setTitle("KarpBot Moderation").setDescription("You have been muted in KingKarp's Dungeon!").addFields({name: 'Reason:', value: `${reason}`},{name: 'Moderator:', value: `${message.author.tag}`}).setFooter("© 2020: KingKarp's Dungeon", 'https://cdn.discordapp.com/avatars/756534705452613634/859c90f9183152e1f131d1ca60ddeaef.png?size=128').setTimestamp();
                    member.send(embed).then(() => {
                        member.roles.add(muterole).then(() => {
                            message.channel.send(`🔈 Successfully muted ${user.tag} with reason: ${reason}`);
                            const logsembed = new Discord.MessageEmbed().setColor('#fffb00').setTitle('Member Muted').addFields({name: 'User:', value: `${member}`},{name: 'Moderator:', value: `${message.author}`},{name: 'Reason:', value: `${reason}`}).setTimestamp().setFooter('KarpBot Moderation');
